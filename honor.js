(function() {
    ...
    const styles = `
        :root { --hr-focus-outline: 0.25rem solid #ffcc00; --hr-link: #0044cc; --hr-badge-bg: #fff59d; --hr-badge-text: #5d4037; --hr-tag-bg: #f0f0f0; --hr-modal-overlay: rgba(0, 0, 0, 0.7); --hr-font: "Iansui", "Microsoft JhengHei", sans-serif; }
        ...
        .hr-btn { background: #fff; border: 2px solid var(--btn-color,#333); color: var(--btn-color,#333); padding: 4px 10px; font-size: 0.95rem; border-radius: 4px; cursor: pointer; font-weight: bold; transition: all 0.2s; display: inline-flex; align-items: center; gap: 6px; }
        .hr-btn:hover { background: rgba(0,0,0,0.05); } .hr-btn.active { background: var(--btn-color,#333); color: #fff; } .hr-btn:focus { outline: none; box-shadow: none; }
        ...
        .hr-title-btn:hover { color: var(--hr-link); text-decoration: underline; } .hr-title-btn:focus { outline: var(--hr-focus-outline); background: #000; color: #ffcc00; text-decoration: underline; }
        ...
        .hr-more { color: var(--hr-link); cursor: pointer; background: none; border: 0; padding: 0; font-size: 0.95rem; } .hr-more:focus { outline: var(--hr-focus-outline); background: #000; color: #ffcc00; }
        .hr-meta { margin-top: auto; display: flex; gap: 8px; font-size: 0.9rem; color: #666; align-items: center; }
        .hr-tag { padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.85rem; cursor: default; } .hr-tag-dept { background: var(--hr-tag-bg); color: #333; }
        .hr-pager { display: flex; justify-content: center; gap: 5px; margin: 25px 0 20px; flex-wrap: wrap; }
        .hr-pg-btn { padding: 8px 12px; border: 1px solid #ddd; background: #fff; color: var(--hr-link); cursor: pointer; border-radius: 4px; font-weight: bold; font-size: 1rem; min-width: 35px;}
        .hr-pg-btn:hover { background: #eee; } .hr-pg-btn.active { background: var(--hr-link); color: #fff; border-color: var(--hr-link); } .hr-pg-btn:disabled { color: #ccc; cursor: not-allowed; } .hr-pg-btn:focus { outline: var(--hr-focus-outline); background: #000; color: #ffcc00; border-color: #ffcc00; }
        ...
        .hr-close { position: absolute; top: 15px; right: 15px; background: #d32f2f; color: #fff; border: 0; width: 32px; height: 32px; border-radius: 4px; font-size: 1.5rem; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 20; }
        .hr-close:hover { background: #b71c1c; } .hr-close:focus { outline: none; background: #b71c1c; }
    ...
    if (!container) { container = document.createElement('div'); container.id = 'honor-root'; document.body.appendChild(container); }
    container.innerHTML = `
        ...
        <div id="hr-modal" class="hr-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="hr-m-title">
            <div class="hr-modal-content">
                ...
                <div class="hr-m-head"><div style="display:flex; align-items:center; gap:15px; flex-wrap:wrap;"><span class="hr-badge"><img src="https://esa.ntpc.edu.tw/web-heromgt/images/trophy.svg" class="hr-badge-icon" alt="">榮譽榜公告</span><div style="color:#888; font-size:0.95rem;"><span id="hr-m-dept"></span> <span id="hr-m-date" style="margin-left:5px;"></span></div></div></div>
                ...
                <div id="hr-m-photos" style="display:none; margin-top:30px;"><div style="margin-bottom:15px;"><span class="hr-badge"><img src="https://esa.ntpc.edu.tw/web-heromgt/images/trophy.svg" class="hr-badge-icon" alt="">競賽照片</span></div><ul id="hr-photo-list" class="hr-photo-list"></ul></div>
            </div>
        </div>
    `;
    ...
    let allData=[], filteredData=[], curPage=1, curCat='all', mcState={}, intervals=[]; let lastFocused = null;
    const el = { list: document.getElementById('hr-list'), load: document.getElementById('hr-loading'), pager: document.getElementById('hr-pager'), filter: document.getElementById('hr-filter'), modal: document.getElementById('hr-modal'), mTitle: document.getElementById('hr-m-title'), mBody: document.getElementById('hr-m-body'), mDept: document.getElementById('hr-m-dept'), mDate: document.getElementById('hr-m-date'), mPhotos: document.getElementById('hr-m-photos'), pList: document.getElementById('hr-photo-list'), mClose: document.getElementById('hr-close') };
    ...
    fetch(DATA_URL).then(r=>r.json()).then(j=>{
        ...
        allData = j.data.map(item => { if (item.dept && CUSTOM_NAME_MAP[item.dept]) item.dept = CUSTOM_NAME_MAP[item.dept]; return item; });
        initFilter(); applyFilter('all');
    }).catch(()=>{ el.load.innerHTML="<span style='color:red'>載入失敗</span>"; });

    function initFilter() {
        const cnt = { "all": allData.length }; allData.forEach(i => cnt[i.category_id] = (cnt[i.category_id]||0)+1); el.filter.innerHTML = "";
        CAT_ORDER.forEach(id => {
            if(CAT_CONF[id] && (id==='all'||(cnt[id]||0)>0)) {
                const btn = document.createElement('button'); btn.className = `hr-btn ${id==='all'?'active':''}`; btn.style.setProperty('--btn-color', CAT_CONF[id].color); btn.title = `篩選分類：${CAT_NAME[id]}，共 ${cnt[id]||0} 則`; btn.innerHTML = `${CAT_NAME[id]} <span class="hr-count">${cnt[id]||0}</span>`;
                btn.onclick = () => { document.querySelectorAll('.hr-btn').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); applyFilter(id); }; el.filter.appendChild(btn);
            }
        });
    }

    function applyFilter(id) { curCat = id; filteredData = id==='all' ? allData : allData.filter(i=>i.category_id==id); curPage = 1; renderPage(1); }

    function renderPage(p) {
        intervals.forEach(clearInterval); intervals=[]; curPage = p; el.list.innerHTML = "";
        if(filteredData.length===0) { el.list.innerHTML="<div style='padding:20px;text-align:center'>無資料</div>"; el.pager.innerHTML=""; return; }
        
        const start = (p-1)*ITEMS_PER_PAGE; const items = filteredData.slice(start, start+ITEMS_PER_PAGE);
        items.forEach((item, idx) => {
            ...
            const catColor = CAT_CONF[item.category_id]?.color || '#999'; const catName = CAT_NAME[item.category_id] || '榮譽';
            ...
            if(item.content && item.content.length>70) { desc = `<div id="${descId}" class="hr-desc hr-trunc">${item.content}</div>`; more = `<button id="${btnId}" class="hr-more" onclick="window.hrMore('${descId}','${btnId}')" title="顯示更多內容">顯示更多...</button>`; }
    ...
    function moveMC(id, d) { const s = mcState[id]; if(!s) return; const div = document.getElementById(id); if(!div) return; const slides = div.querySelectorAll('.hr-mc-slide'); slides[s.c].classList.remove('active'); s.c = (s.c + d + s.t) % s.t; slides[s.c].classList.add('active'); }
    ...
    window.hrToggle = (id,e) => { e.stopPropagation(); const s = mcState[id], btn = e.currentTarget; if(s.p) { s.p=false; stopMC(id); btn.innerText="▶"; btn.setAttribute('aria-label', "播放輪播"); btn.title="播放輪播"; } else { s.p=true; startMC(id); btn.innerText="⏸"; btn.setAttribute('aria-label', "暫停輪播"); btn.title="暫停輪播"; } };
    window.hrMore = (d,b) => { document.getElementById(d).classList.remove('hr-trunc'); document.getElementById(b).style.display='none'; };
    
    window.hrOpen = (idx) => {
        const i = filteredData[idx]; lastFocused = document.activeElement;
    ...
    function renderPager() {
        ...
        if(s>1) mkBtn("<<", false, s-1, "上 10 頁"); mkBtn("<", curPage===1, curPage-1, "上一頁");
        for(let i=s; i<=e; i++) mkBtn(i, false, i);
        mkBtn(">", curPage===tot, curPage+1, "下一頁"); if(e<tot) mkBtn(">>", false, e+1, "下 10 頁");
    }
})();