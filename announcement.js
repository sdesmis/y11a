(function() {
    ...
    const styles = `
        :root {
            ...
            --ann-tag-bg: #f0f0f0; --ann-font: "Iansui", "Microsoft JhengHei", "Heiti TC", sans-serif;
        }
    ...
    container.innerHTML = `
        <div class="ann-container">
            ...
            <div id="ann-toolbar" class="ann-toolbar">
                ...
                <button id="ann-toggle-search" class="ann-search-btn" aria-expanded="false" title="開啟/關閉搜尋面板">
                    <svg aria-hidden="true" focusable="false" style="width:16px;height:16px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg> 資料檢索
                </button>
            </div>
            <div id="ann-search-panel" class="ann-search-panel">
                <div class="ann-search-row">
                    <div class="ann-search-item"><label for="ann-s-start">公告日期：</label><input type="date" id="ann-s-start" class="ann-form-control" title="起始日期"><label for="ann-s-end" class="ann-sr-only">至</label><span aria-hidden="true" style="margin:0 5px;">至</span><input type="date" id="ann-s-end" class="ann-form-control" title="結束日期"></div>
                    <div class="ann-search-item"><label for="ann-s-cat">類別：</label><select id="ann-s-cat" class="ann-form-control" title="選擇類別"><option value="">全部類別</option></select></div>
                </div>
                <div class="ann-search-row">
                    <div class="ann-search-item" style="flex-grow:1;"><label for="ann-s-dept">處室：</label><select id="ann-s-dept" class="ann-form-control" title="選擇處室"><option value="">全部處室</option></select></div>
                    <div class="ann-search-item" style="flex-grow:2;"><label for="ann-s-key">關鍵字：</label><input type="text" id="ann-s-key" class="ann-form-control" placeholder="請輸入標題或內容關鍵字" title="請輸入關鍵字"></div>
                    <div class="ann-search-item"><button id="ann-btn-search" class="ann-submit" title="執行查詢">🔍 複合查詢</button><button id="ann-btn-reset" class="ann-reset" title="重置搜尋條件">重置</button></div>
                </div>
            </div>
    ...
    function renderPager() {
        ...
        if(s>1) mkBtn("<<", false, s-1, "上 10 頁"); 
        mkBtn("<", currentPage===1, currentPage-1, "上一頁");
        for(let i=s; i<=e; i++) mkBtn(i, false, i);
        mkBtn(">", currentPage===total, currentPage+1, "下一頁"); 
        if(e<total) mkBtn(">>", false, e+1, "下 10 頁");
    }
