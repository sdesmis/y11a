(function() {
    ...
    const styles = `
        :root {
            ...
            --ann-focus-outline: 0.25rem solid #ffcc00; --ann-modal-overlay: rgba(0, 0, 0, 0.7);
            --ann-tag-bg: #f0f0f0; 
            /* ★ 確保 Iansui 排第一順位 ★ */
            --ann-font: "Iansui", "Microsoft JhengHei", "Heiti TC", sans-serif;
        }
        #announcement-root { font-family: var(--ann-font); width: 100%; line-height: 1.5; font-size: 1rem; color: var(--ann-text-color); }
        #announcement-root * { box-sizing: border-box; font-family: inherit; } /* 強制繼承字型 */
        ...
        /* 強制限制圖片高度 */
        .ann-icon-img { 
            height: 18px !important; width: auto !important; max-width: 40px !important;
            vertical-align: middle !important; border: none; display: inline-block;
        }
        ...
        .ann-modal-overlay { 
            ...
            background: var(--ann-modal-overlay); display: none; 
            justify-content: center; align-items: center; z-index: 99999; 
        }
        .ann-modal-content { 
            background: #fff; width: 90%; max-width: 70ch; max-height: 90vh; 
            overflow-y: auto; padding: 1.5rem; border: 2px solid #000; border-radius: 0.5rem; position: relative; 
        }
        .ann-close-btn { 
            position: absolute; top: 15px; right: 15px; background: #d32f2f; color: #fff; border: none; 
            width: 32px; height: 32px; border-radius: 4px; font-size: 1.5rem; padding: 0; 
        ...
        @media (max-width: 700px) {
            .ann-item { flex-direction: column; align-items: flex-start; gap: 0.3rem; }
            .ann-date { width: auto; font-size: 0.9rem; margin-right: 10px; display: inline-block; }
            .ann-tags { margin-bottom: 4px; }
            .ann-search-row { flex-direction: column; align-items: stretch; }
        }
    `;
    ...
    if (!container) {
        console.warn('找不到 id="announcement-root"，自動建立。');
        container = document.createElement('div');
        container.id = 'announcement-root';
    ...
    container.innerHTML = `
        <div class="ann-container">
            ...
            <div id="ann-toolbar" class="ann-toolbar">
                <nav id="ann-filter-group" class="ann-filter-group" aria-label="公告分類快速篩選"><span class="ann-sr-only">載入分類中...</span></nav>
                <button id="ann-toggle-search" class="ann-search-btn" aria-expanded="false">
            ...
            <div id="ann-search-panel" class="ann-search-panel">
                <div class="ann-search-row">
                    <div class="ann-search-item"><label for="ann-s-start">公告日期：</label><input type="date" id="ann-s-start" class="ann-form-control"><label for="ann-s-end" class="ann-sr-only">至</label><span aria-hidden="true" style="margin:0 5px;">至</span><input type="date" id="ann-s-end" class="ann-form-control"></div>
                    <div class="ann-search-item"><label for="ann-s-cat">類別：</label><select id="ann-s-cat" class="ann-form-control"><option value="">全部類別</option></select></div>
                </div>
                <div class="ann-search-row">
                    <div class="ann-search-item" style="flex-grow:1;"><label for="ann-s-dept">處室：</label><select id="ann-s-dept" class="ann-form-control"><option value="">全部處室</option></select></div>
                    <div class="ann-search-item" style="flex-grow:2;"><label for="ann-s-key">關鍵字：</label><input type="text" id="ann-s-key" class="ann-form-control" placeholder="請輸入標題或內容關鍵字"></div>
                    <div class="ann-search-item"><button id="ann-btn-search" class="ann-submit">🔍 複合查詢</button><button id="ann-btn-reset" class="ann-reset">重置</button></div>
                </div>
            </div>
        ...
        <div id="ann-modal" class="ann-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="ann-m-title">
            <div class="ann-modal-content">
                <button id="ann-m-close" class="ann-close-btn" title="關閉">&times;</button>
                <div style="border-bottom: 2px solid #eee; margin-bottom: 1rem; padding-bottom: 0.5rem; padding-right: 30px;">
                    <h2 id="ann-m-title" style="margin:0 0 10px 0; font-weight:normal;">公告詳細內容</h2>
                    <div style="color:#555; font-size: 0.95rem;"><span id="ann-m-dept"></span></div>
                    <div id="ann-m-period" style="color:#777; font-size: 0.9rem; margin-top: 5px;"></div>
                </div>
    ...
    const DEPT_LIST = ["校長室", "教務處", "學務處", "總務處", "輔導處", "人事室", "主計室", "幼兒園"];
    ...
    fetch(DATA_URL).then(r=>r.json()).then(json=>{
        ...
        allData = json.data;
        if(LOCKED_CATEGORY_ID) { el.toolbar.style.display='none'; el.searchPanel.style.display='none'; applyFilter(LOCKED_CATEGORY_ID); } 
        else { initFilters(); initSearch(); filteredData = allData; renderPage(1); }
    ...
    function createBtn(id, count) {
        ...
        btn.className = `ann-filter-btn ${id==='all'?'active':''}`;
        btn.style.setProperty('--btn-color', color);
        btn.title = `篩選：${name}，共 ${count} 則`;
        btn.innerHTML = `${name} <span class="ann-count">${count}</span>`;
        btn.onclick = () => {
            document.querySelectorAll('.ann-filter-btn').forEach(b=>b.classList.remove('active'));
            btn.classList.add('active'); applyFilter(id); el.searchPanel.style.display='none';
        };
    ...
    function doSearch() {
        ...
        filteredData = allData.filter(i => {
            let m = true;
            if(sd||ed) { const idt = parseRoc(i.date); if(idt){ if(sd&&idt<sd)m=false; if(ed&&idt>ed)m=false; } }
            if(m&&c && (!i.category_ids || !i.category_ids.includes(c))) m=false;
            if(m&&d && i.department!==d) m=false;
            if(m&&k && !i.title.toLowerCase().includes(k)) m=false;
            return m;
        });
    ...
    el.btnReset.onclick = () => { el.sStart.value=el.sEnd.value=el.sCat.value=el.sDept.value=el.sKey.value=""; applyFilter('all'); document.querySelector('.ann-filter-btn').classList.add('active'); };
    ...
    function renderPage(p) {
        ...
        pageItems.forEach((item, idx) => {
            ...
            let tags = "";
            if(item.category_ids) item.category_ids.forEach(cid => { if(CAT_CONFIG[cid]) tags += `<span class="ann-tag" style="background:${CAT_CONFIG[cid].color};color:#fff">${CAT_MAP[cid]}</span>`; });
            if(!tags) tags = `<span class="ann-tag ann-tag-dept">${item.department}</span>`;
            let titleHtml = "";
            if(item.pub_view == "2") { titleHtml = `<a href="https://esa.ntpc.edu.tw" target="_blank" class="ann-title-btn">${item.is_top==1?IMG_TOP:''}${item.title}${checkIsNew(item.date)?IMG_NEW:''}</a> <a href="https://esa.ntpc.edu.tw" target="_blank" class="ann-login">須登入</a>`; } 
            else { titleHtml = `<button class="ann-title-btn" onclick="window.annOpen(${start+idx})">${item.is_top==1?IMG_TOP:''}${item.title}${checkIsNew(item.date)?IMG_NEW:''}</button>`; }
    ...
    function renderPager() {
        ...
        const mkBtn = (txt, dis, go) => { const b = document.createElement('button'); b.className=`ann-page-btn ${txt==currentPage?'active':''}`; b.innerText=txt; b.disabled=dis; b.onclick=()=>renderPage(go); el.pagination.appendChild(b); };
        const gp = 10, curGp = Math.ceil(currentPage/gp), s = (curGp-1)*gp+1, e = Math.min(s+gp-1, total);
        if(s>1) mkBtn("<<", false, s-1); mkBtn("<", currentPage===1, currentPage-1);
        for(let i=s; i<=e; i++) mkBtn(i, false, i); mkBtn(">", currentPage===total, currentPage+1); if(e<total) mkBtn(">>", false, e+1);
    }

    window.annOpen = function(idx) {
        const item = filteredData[idx]; lastFocused = document.activeElement;
        el.mTitle.innerText = item.title;
        el.mDept.innerText = item.job_title ? `${item.department} / ${item.job_title}` : item.department;
        ...
        if(item.link_a||item.link_b) {
            ...
            if(item.link_a) lh += `<li><a href="${item.link_a}" target="_blank">參考網址1</a></li>`;
            if(item.link_b) lh += `<li><a href="${item.link_b}" target="_blank">參考網址2</a></li>`;
            el.mLinkList.innerHTML = lh;
        } else el.mLinks.style.display='none';
        if(item.attachments && item.attachments.length>0) {
            el.mAttach.style.display='block'; el.mFileList.innerHTML = item.attachments.map(f=>`<li><a href="${f.url}" target="_blank">下載：${f.name}</a></li>`).join('');
    ...
    document.addEventListener('keydown', (e) => { if(e.key==='Escape' && el.modal.style.display==='flex') close(); });
})();